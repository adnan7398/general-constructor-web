...
const createAuditLog = async (entityId, action, actorId, changes, metadata) => {
  try {
...
reportRoutes.patch('/:id/submit', async (req, res) => {
  try {
    ...
    // Only draft or rejected/revision-required can be submitted
    if (!['draft', 'rejected', 'revision-required'].includes(report.workflow.currentStep)) {
      return res.status(400).json({ error: 'Report cannot be submitted in current state' });
    }

    report.workflow.currentStep = 'submitted_for_review';
    report.workflow.history.push({
      action: 'submitted',
      by: req.user._id,
      comments: req.body.comments || 'Submitted for review'
    });
    report.status = 'submitted'; // Legacy compatibility
    report.lastUpdated = new Date();

    await report.save();

    // Audit Log
    await createAuditLog(report._id, 'SUBMIT', req.user._id, [{
      field: 'status', oldValue: 'draft', newValue: 'submitted_for_review'
    }], { ipAddress: req.ip });
...
reportRoutes.patch('/:id/approve', async (req, res) => {
  try {
    // RBAC: Only Manager or Super Admin
    if (!['MANAGER', 'SUPER_ADMIN'].includes(req.user.role?.toUpperCase()) && req.user.role !== 'Client') {
      return res.status(403).json({ error: 'Access denied: Approver role required' });
    }

    const report = await Report.findById(req.params.id);
    if (!report) return res.status(404).json({ error: 'Report not found' });

    report.workflow.currentStep = 'approved';
    report.workflow.history.push({
      action: 'approved',
      by: req.user._id,
    ...
    report.approvedBy = req.user.name;
    report.approvedAt = new Date();
    report.lastUpdated = new Date();

    // Snapshot budget at approval time if provided
    if (req.body.budgetSnapshot) {
      report.budgetSnapshots = req.body.budgetSnapshot;
    }

    await report.save();

    await createAuditLog(report._id, 'APPROVE', req.user._id, [{
...
// Reject report
reportRoutes.patch('/:id/reject', async (req, res) => {
  try {
    // RBAC: Only Manager or Super Admin
    if (!['MANAGER', 'SUPER_ADMIN'].includes(req.user.role?.toUpperCase()) && req.user.role !== 'Client') {
      return res.status(403).json({ error: 'Access denied: Approver role required' });
    ...
    report.workflow.currentStep = 'rejected';
    report.workflow.history.push({
      action: 'rejected',
      by: req.user._id,
      comments: reason
    ...
    await createAuditLog(report._id, 'REJECT', req.user._id, [{
      field: 'status', oldValue: 'submitted', newValue: 'rejected'
    }], { ipAddress: req.ip, reason });
